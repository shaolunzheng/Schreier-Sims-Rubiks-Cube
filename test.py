...
table[0,0] = np.array([0,0,1])
table[1,0] = np.array([2,0,1])
table[2,1] = np.array([0,1,1])
table[2,2] = np.array([4,0,1])
...
from functions import compose, inverse, schreiers_vector, generating_set, sims_filter
...
schreiers_vector = schreiers_vector(temp_generators, n, k)
